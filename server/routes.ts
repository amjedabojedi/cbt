...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/export/pdf", authenticate, async (req, res) => {
    try {
      ...
      // Prepare and add data based on the export type
      switch (type) {
        case "emotions":
          const emotions = await storage.getEmotionRecordsByUser(targetUserId);
          ...
          if (thoughts && thoughts.length > 0) {
            thoughts.forEach((record, index) => {
              ...
              doc.fontSize(10)
                 ...
                 .text('Situation:')
                 .font('Helvetica')
                 .text(record.situation || 'None recorded', { indent: 20 })
                 .moveDown(0.5);
              ...
              doc.font('Helvetica-Bold')
                 .text('Emotions:')
                 .font('Helvetica')
                 .text(record.emotions || 'None recorded', { indent: 20 })
                 .moveDown(0.5);
              
              doc.font('Helvetica-Bold')
                 .text('Distortions:')
                 .font('Helvetica')
                 .text(record.distortions?.join(', ') || 'None identified', { indent: 20 })
                 .moveDown(0.5);
              
              doc.font('Helvetica-Bold')
                 .text('Rational Response:')
                 .font('Helvetica')
                 .text(record.rationalResponse || 'None recorded', { indent: 20 })
                 .moveDown(0.5);
              
              if (record.behavioralPlan) {
                doc.font('Helvetica-Bold')
                   .text('Behavioral Plan:')
                   .font('Helvetica')
                   .text(record.behavioralPlan, { indent: 20 })
                   .moveDown(0.5);
              }
        ...
        case "journals":
          ...
          if (journals && journals.length > 0) {
            journals.forEach((entry, index) => {
              ...
              if (entry.emotions) {
                const emotionsList = typeof entry.emotions === 'string' 
                  ? entry.emotions 
          ...
          if (goals && goals.length > 0) {
            goals.forEach((goal, index) => {
              ...
              doc.font('Helvetica-Bold')
                 .text('Specific:')
                 .font('Helvetica')
                 .text(goal.specific || 'Not specified', { indent: 20 })
                 .moveDown(0.25);
              
              doc.font('Helvetica-Bold')
                 .text('Measurable:')
                 .font('Helvetica')
                 .text(goal.measurable || 'Not specified', { indent: 20 })
                 .moveDown(0.25);
              
              doc.font('Helvetica-Bold')
                 .text('Achievable:')
                 .font('Helvetica')
                 .text(goal.achievable || 'Not specified', { indent: 20 })
                 .moveDown(0.25);
              
              doc.font('Helvetica-Bold')
                 .text('Relevant:')
                 .font('Helvetica')
                 .text(goal.relevant || 'Not specified', { indent: 20 })
                 .moveDown(0.25);
              
              doc.font('Helvetica-Bold')
                 .text('Time-bound:')
                 .font('Helvetica')
                 .text(goal.timebound || 'Not specified', { indent: 20 })
                 .moveDown(0.5);
        ...
        case "all":
          // Title page for all data
          doc.fontSize(18)
             .font('Helvetica-Bold')
             .text('Complete Data Export', { align: 'center' })
             .moveDown(0.5);
          
          doc.fontSize(12)
             .font('Helvetica')
             .text(`This document contains all data for ${userName}`, { align: 'center' })
             .moveDown(0.5);
          ...
          doc.fontSize(12)
             .text('1. Emotion Records')
             .moveDown(0.25);
          
          doc.text('2. Thought Records')
             .moveDown(0.25);
          
          doc.text('3. Journal Entries')
             .moveDown(0.25);
          
          doc.text('4. Goals')
             .moveDown(2);
          
          // Start with emotions section
          doc.addPage();
          
          // Emotions Section
          const allEmotions = await storage.getEmotionRecordsByUser(targetUserId);
          ...
          if (allEmotions && allEmotions.length > 0) {
            ...
            // Table rows
            allEmotions.forEach((record, index) => {
              // Add page break if needed
              if (doc.y > doc.page.height - 100) {
                doc.addPage();
                ...
                doc.fontSize(10)
                   ...
                   .text('Core Emotion', 150, 70, { width: 100 })
                   .text('Primary Emotion', 250, 70, { width: 100 })
                   .text('Intensity', 350, 70, { width: 50 })
                   .moveDown(0.5);
              ...
              // Add a thin separator line between records
              if (index < allEmotions.length - 1) {
                doc.moveTo(50, doc.y)
                   .lineTo(doc.page.width - 50, doc.y)
                   .stroke({ dash: [2, 2] })
                   .moveDown(0.5);
          ...
          doc.addPage();
          const allThoughts = await storage.getThoughtRecordsByUser(targetUserId);
          ...
          if (allThoughts && allThoughts.length > 0) {
            allThoughts.forEach((record, index) => {
              const date = new Date(record.createdAt).toLocaleDateString();
              ...
              // Add page break if needed
              if (doc.y > doc.page.height - 200 && index < allThoughts.length - 1) {
                doc.addPage();
              }
            });
          } else {
          ...
          doc.addPage();
          const allJournals = await storage.getJournalEntriesByUser(targetUserId);
          ...
          if (allJournals && allJournals.length > 0) {
            allJournals.forEach((entry, index) => {
              const date = new Date(entry.createdAt).toLocaleDateString();
              ...
              // Add page break if needed
              if (doc.y > doc.page.height - 200 && index < allJournals.length - 1) {
                doc.addPage();
              }
            });
          } else {
          ...
          // Goals Section
          doc.addPage();
          const allGoals = await storage.getGoalsByUser(targetUserId);
          ...
          if (allGoals && allGoals.length > 0) {
            allGoals.forEach((goal, index) => {
              ...
              // Add page break if needed
              if (doc.y > doc.page.height - 200 && index < allGoals.length - 1) {
                doc.addPage();
              }
            });
          } else {
        ...
        default:
          return res.status(400).json({ message: "Invalid export type for PDF format" });
      }
