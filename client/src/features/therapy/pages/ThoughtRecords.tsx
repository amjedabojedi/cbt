...
export default function ThoughtRecords() {
  ...
  const pageTitle = isViewingClientData ? "Client Thoughts" : "Thought Records";
  const pageSubtitle = isViewingClientData
    ? "Review automatic thoughts, cognitive distortions, and reframing exercises for this client."
    : "Record your thoughts and identify cognitive distortions to better understand your thinking patterns.";
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-purple-400/80 text-xs font-bold tracking-widest uppercase">
                    {isViewingClientData ? "Clinical Review" : "Cognitive Reframing"}
                  </span>
                </div>
              ...
              {thoughtRecords.length > 0 && (
                <div className="flex items-center gap-6 shrink-0 flex-wrap">
                  {[
                    { value: totalThoughts, label: "Total Logs" },
                    { value: challengedThoughts, label: "Challenged" },
                    { value: unchallengedThoughts, label: "Unchallenged" },
                  ].map((s, i) => (
            ...
            <div className="mt-6 bg-white/5 rounded-xl border border-white/10 p-4">
              <div className="flex items-center justify-between mb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-purple-200 uppercase tracking-widest">Thought Challenge Rate</span>
                </div>
                <span className="text-xs text-purple-400">{totalThoughts > 0 ? Math.round((challengedThoughts / totalThoughts) * 100) : 0}% challenged</span>
              </div>
              ...
              <p className="text-[11px] text-purple-400/60 mt-1.5">{challengedThoughts} of {totalThoughts} thoughts challenged — reframing builds cognitive flexibility.</p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto pb-10 pt-6 space-y-5">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className={isRecordTab ? "space-y-3" : "space-y-5"}>
            ...
            <TabsContent value="history" className="mt-0">
              <div className="space-y-4">
                <SectionLabel
                  ...
                  label={isViewingClientData ? "Client Thought History" : "Thought History"}
                />
            ...
            <TabsContent value="insights" className="mt-0">
              <div className="space-y-4">
                <SectionLabel icon={<TrendingUp className="h-4 w-4" />} label="Thought Insights" />
