...
export default function ReflectionWizard({ emotion, open, onClose }: ReflectionWizardProps) {
  ...
  const handleCloseAttempt = (open: boolean) => {
    // If dialog is being closed and we have unsaved data
    if (!open && form.formState.isDirty) {
      // Ask for confirmation
      const confirmClose = window.confirm(
        "You have unsaved changes. Are you sure you want to close this reflection? Your progress will be lost."
      );
  ...
  return (
    <Dialog open={open} onOpenChange={handleCloseAttempt}>
      <DialogContent 
        className="sm:max-w-[800px] md:max-w-[900px] p-6 max-h-[90vh] overflow-y-auto"
        onEscapeKeyDown={(e) => {
          // Prevent closing with Escape key if dirty
